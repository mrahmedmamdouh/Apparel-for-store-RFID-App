...
import co.kr.bluebird.newrfid.app.bbrfidbtdemo.fragment.InventoryFragment;
...
public class FileManager {
    ...
    private static final String mLogFileNameOR = "/OR-";
    private static final String mExtentionName = ".csv";
    private static final String mExtentionNameOR = ".txt";
    ...
    public boolean openFile(String name) {
        ...
        mFile = new File(mDir, mLogFileName + date +"_"+ name + mExtentionName);
        if (readyToWrite()) {
            clearTagList();
    ...
    public boolean openFileInv(String name) {
        ...
        mFile = new File(mDir, mLogFileNameOR + date +"_"+ name + mExtentionNameOR);
        if (readyToWrite()) {
            clearTagList();
            mTagList = new CopyOnWriteArrayList<>();
            mDoLoop = true;
                        ...
                        if (mTagList.size() > 0) {
                            for (String tag : mTagList) {
                                try {
                                    ...
                                    String sub = tag.substring(4);
                                    if (sub.substring(0,3) == "3000"){return;}
                                        else{
                                    mWriter.println(sub + "\r\n");
                                    mTagList.remove(sub);}
                                } catch (java.lang.Exception e) {
